...
namespace NexusForever.WorldServer.Network.Message.Handler.Chat
{
    public class ClientWhoRequestHandler : IMessageHandler<IWorldSession, ClientWhoRequest>
    {
        ...
        //{
            ...
            //List<ServerWhoResponse.WhoPlayer> whoResponsePlayerList = worldSessions.Aggregate(new List<ServerWhoResponse.WhoPlayer>(), AddPlayerToList);
            //// Iterate over sessions (connected clients) for filtering.
            //foreach (IWorldSession sessionCandidate in worldSessions)
            //{
            //    if (sessionCandidate.Player == null)
            //    {
            //        // We want to pass this iteration if the session isn't logged into a character.
            //        continue;
            //    }
            ...
            //    {
            //        if (sessionCandidate.Player.GuildManager.Guild == null)
            //        {
            //            // Player may not have a guild, so we want to continue here if that's the case.
            //            continue;
            //        }
        ...
        public void HandleMessage(IWorldSession requestingSession, ClientWhoRequest request)
        {
            ...
            List<ServerWhoResponse.WhoPlayer> whoResponsePlayerList = worldSessions.Aggregate(new List<ServerWhoResponse.WhoPlayer>(), FilterOutNull(AddPlayerToList));
        ...
        private List<ServerWhoResponse.WhoPlayer> AddPlayerToList(List<ServerWhoResponse.WhoPlayer> whoResponsePlayerList, IWorldSession sessionCandidate)
        {
            if (requestingSession.Id == sessionCandidate.Id && (shouldSearchesIncludeThePlayerInitiatingSearch == false))
            {
                // This code exits early if searching party's session ID is matched with one of the filtered client session's IDs.
                return whoResponsePlayerList;
            }
        ...
        private Func<List<ServerWhoResponse.WhoPlayer>, IWorldSession, List<ServerWhoResponse.WhoPlayer>> FilterOutNull(Func<List<ServerWhoResponse.WhoPlayer>, IWorldSession, List<ServerWhoResponse.WhoPlayer>> reducer)
        {
            return (whoResponsePlayerList, sessionCandidate) =>
            {
                if (sessionCandidate.Player == null)
                {
                    // We want to pass this iteration if the session isn't logged into a character.
                ...
                return reducer(whoResponsePlayerList, sessionCandidate);
            };
        }
    }
}
